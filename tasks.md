...
- [ ] P1: Reconcile `docs/daz-chatbot-architecture.md` logger-rule description with SQL plugin behavior (docs say rules are stored but unused; code subscribes after DB connect).
- [ ] P1: Stabilize or gate time-sensitive eventbus priority tests (reduce sleeps, make deterministic, or guard with `-short`).
