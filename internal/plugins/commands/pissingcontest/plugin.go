...
func (p *Plugin) handlePissCommand(channel, username string, args []string) {
	...
	if !p.isBotTarget(channel, target) {
		inRoom, err := p.isUserInRoom(channel, target)
		if err != nil {
			logger.Warn(p.name, "failed to verify chat presence for -%s in %s: %v", target, channel, err)
		} else if !inRoom {
			p.sendCommandResult(channel, username, fmt.Sprintf("-%s ain't in chat right now mate", target), false)
			return
		}
	}
...
func (p *Plugin) isUserInRoom(channel, username string) (bool, error) {
	...
	rows, err := p.sqlClient.QueryContext(ctx, query, room, normalized)
	if err != nil {
		return false, err
	}
	defer func() {
		if err := rows.Close(); err != nil {
			logger.Error(p.name, "failed to close presence rows: %v", err)
		}
	}()
